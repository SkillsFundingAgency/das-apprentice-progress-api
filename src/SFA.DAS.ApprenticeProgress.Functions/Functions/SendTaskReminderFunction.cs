        ...
        public async Task Run([TimerTrigger("0 */1 * * * *", RunOnStartup = true)] TimerInfo timer, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (taskReminders.TaskReminders.Count > 0)
                {
                    ...
                    {
                        await _messageService.SendMessage(new SendPushNotificationCommand { ApprenticeAccountIdentifier = reminder.ApprenticeAccountId , Body = reminder.Title, Title = "Task due " + reminder.DueDate.ToString() });
